...
class SentimentClassifier(nn.Module):
    def __init__(self, n_classes, pre_trained_model_name='bert-base-uncased', dropout_prob=0.1):
        ...
        target_dim = self.bert_hidden_size + self.resnet_output_size 

        self.bert_projection = nn.Linear(self.bert_hidden_size, target_dim)
        self.resnet_projection = nn.Linear(self.resnet_output_size, target_dim)

        self.cross_attention = nn.MultiheadAttention(embed_dim=target_dim, num_heads=8, dropout=dropout_prob)

        self.drop = nn.Dropout(p=dropout_prob)
        self.fc = nn.Linear(target_dim, n_classes)

    def forward(self, input_ids, attention_mask, image):
        bert_outputs = self.bert(input_ids=input_ids, attention_mask=attention_mask)
        text_features = bert_outputs.last_hidden_state  # [batch_size, seq_length, hidden_size]

        resnet_features = self.resnet(image)  # [batch_size, resnet_output_size (512)]

        text_features = self.bert_projection(text_features)  # [batch_size, seq_length, target_dim]
        resnet_features = self.resnet_projection(resnet_features)  # [batch_size, target_dim]

        text_features = text_features.permute(1, 0, 2)  # [seq_length, batch_size, target_dim]
        resnet_features = resnet_features.unsqueeze(0)  # [1, batch_size, target_dim], 为了匹配维度

        attn_output, _ = self.cross_attention(resnet_features, text_features, text_features)

        attn_output = attn_output.mean(dim=0)  # [batch_size, target_dim]

        x = self.drop(attn_output)
...
with open(f'./resnet_bert_base_17_img2text.json', 'w+') as f:
